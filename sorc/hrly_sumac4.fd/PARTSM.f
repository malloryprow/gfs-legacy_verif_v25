...
C                .      .    .                                       .  
C SUBPROGRAM:    PARTSM      SET UP AND COMPUTE FORECAST-MINUS-OBS.     
C   PRGMMR: STACKPOLE        ORG: W/NP12    DATE: 96-06-25 
C                                                                       
C ABSTRACT:  INTERPOLATES ALL VALID FORECAST FIELDS TO LOCATION OF      
...
C   98-07-14  VLCEK       COMPILE IN F90 AND MAKE Y2K COMPATIBLE.
C                                                                       
C USAGE:    CALL PARTSM                                                 
...
C     REMAINING GQ VALUES ARE RESERVED FOR FUTURE USE.                
     ...
     &                IUJET , IUSTA    , IUOUT  , IUOPN, IUTAB
      ...
      INTEGER      QWANT , NFILE , IFCDAT(4) , IPP(GNLEV), PLEV, IRET
...
C     NEXT DO IS OVER DESIRED LEVELS                               
...
C
C *** DIAGNOSTIC PRINT STATEMENT FOLLOWS
C
      IF (DIAG)    THEN
        PRINT 2222, IRET, KI, KR, KPDSI(14), KPDSI(7), KPDSI(5),
     &              (DATA(I),I=1900,1905)
 2222   FORMAT ( 1X, I3, 5I8, 6F10.2)
      ENDIF
C
      IF(IRET.NE.0) THEN
         PRINT 9990
...
C        CURRENTLY Y2K-COMPLIANT, AS IS GVALDT (ABOVE)
...
C     VECTOR/SCALAR DONE SEPARATELY                            
      ...
      IF(.NOT.VECTOR) THEN
         DO 100 NS=1,JSTA
         X = ZTRUVL(NS)
         IF(STI(NS).LT.F9-1..AND.STJ(NS).LT.F9-1.
     &      .AND.ABS(X).LT.F9-1.) THEN
            ...
            IF(INP.GT.0) THEN
               XNP   = ZTRUVL(INP)
               STINP = STVL(INP)
               IF(ABS(XNP).LT.F9-1..AND.ABS(STINP).LT.F9-1.) THEN
                  DI                 = 1./DISTNN(NS)
  ...
  100    CONTINUE
      ELSE
         DO 120 NS=1,JSTA
         IF(STI(NS).LT.F9-1..AND.STJ(NS).LT.F9-1.) THEN
            UO = ZTRUVL(NS)
            VO = ZTRUV1(NS)
            IF(ABS(UO).LT.F9-1..AND.ABS(VO).LT.F9-1.) THEN
               UF = STVL(NS)
               VF = STV1(NS)
               SO = SQRT(UO**2 + VO**2)
               SF = SQRT(UF**2 + VF**2)
