...
C   PASSED BY COMMON BLOCK /MAPINF/.   51 STATEMENTS.
C                                                                       
C ATTRIBUTES:                                                           
...
C     FOUND IN SUBROUTINE ACCUMU.                                    
     ...
     &                NCYCMP     , F9, DATENL
      ...
      DIMENSION  MAPLST(GNMAPS), ROTANG(GNMAPS)
      DIMENSION  UG(GMXSTN)    , VG(GMXSTN)
      LOGICAL    MAP

      DATA MAPLST /2,3,5,6,21,22,26,27,28,29,30,33,34,45,46,104,254/
      DATA ROTANG /0.,0.,15.,15.,0.,0.,15.,-10.,-10.,6*0.,15.,0. /
...
C     CHECK KMAP AGAINST EXISTING MAP TYPES.                         
   ...
   30 CONTINUE
     ...
     &   KMAPT.EQ.6.OR.(KMAPT.GT.9.AND.KMAPT.LE.15).OR.
     &   KMAPT.EQ.17) GO TO 230
